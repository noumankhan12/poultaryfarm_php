<?php
    ...
    if(isset($_GET["birdspurchdelete"])){
      ...
      $medPurchaseObject = new CrudOperation();
      ...
      if(isset($_POST["medicinepurchsave"])){
          ...
          // Call the insertion method to add record to the database
          if($medPurchaseObject->insertionMethod("MedicinePurchase", $myArray)){
              header("location: ../medicinePurchase.php?msg=Insertion was successfull!");
          };
      }
      // Handle the edit button for record editing
      if(isset($_POST["medicinepurchedit"])){
          $id = $_POST["id"];
          ...
          if($medPurchaseObject->updateMethod("MedicinePurchase", $where, $myArray)){
              header("location: ../medicinePurchase.php?msg=Updated Successfully!");
          }
      }
  
      // Check if delete button was triggered
      if(isset($_GET["medicinepurchdelete"])){
          $id = $_GET["id"] ?? null;
          $where = array("MedicinePurchase_ID" => $id);
          if($medPurchaseObject->deleteMethod("MedicinePurchase", $where)){
              header("location: ../medicinePurchase.php?msg=Record deleted successfully!");
          }
      }
    // BIRDS MORTALITY
